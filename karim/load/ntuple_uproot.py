...
class InputFile(object):
    ...
    def __init__(self, filename, friendTrees = [], treeName = "Events"):
        ...
        for ft in friendTrees: # might become a problem
...
class TreeIterator:
    ...
    def __next__(self):
        if self.idx%self.pstep==0 and self.idx>0:
            ...
            print("  time for {} events:  {:.1f} s".format(self.pstep, self.stop - self.start))
            print("  estimated time for {} events: {:.0f} min".format(
                self.scale, (self.stop-self.start)/self.pstep*self.scale/60.))
            self.start = time.perf_counter()

        if self.idx < self.max:
            self.tree.arrays()[idx] #ToDo anpassen
            self.idx+=1
