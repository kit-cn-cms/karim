...
def calculate_variables(df):
    ...
    vectors = common.Vectors(df, name, ["HadTopB", "LepTopB"]) # new
    vectors.add(["HadTopB", "LepTopB"], out = "HadTopBLepTopB")

    df[name+"_HadTopBLepTopB_dPhi"] = common.get_dPhi(df[name+"_HadTopB_Phi"].values, df[name+"_LepTopB_Phi"])
    df[name+"_HadTopBLepTopB_dEta"] = abs(df[name+"_HadTopB_Eta"].values - df[name+"_LepTopB_Eta"])
    #df[name+"_HadTopBLepTopB_dPt"] = abs(df[name+"_HadTopB_Pt"].values - df[name+"_LepTopB_Pt"])
    df[name+"_HadTopBLepTopB_dR"]   = np.sqrt(df[name+"_HadTopBLepTopB_dPhi"].values**2 + df[name+"_HadTopBLepTopB_dEta"].values**2)
    # df[name+"_HadTopBLepTopB_dKin"] = np.sqrt( (df[name+"_HadTopBLepTopB_dPhi"].values/(2.*np.pi))**2 + \
    #                                (df[name+"_HadTopBLepTopB_dEta"].values/5.)**2 + \
    #                                (df[name+"_HadTopBLepTopB_dPt"].values/1000.)**2 )

    vectors.initIndexedVector(df, "TightLepton", 0)
    vectors.add(["TightLepton_0", "LepTopB"], out = "LepTopBTightLepton")
    vectors.add(["TightLepton_0", "HadTopB"], out = "HadTopBTightLepton")

    df[name+"_HadTopBTightLepton_dPhi"] = common.get_dPhi(df[name+"_HadTopB_Phi"].values, df["TightLepton_Phi[0]"])
    df[name+"_HadTopBTightLepton_dEta"] = abs(df[name+"_HadTopB_Eta"].values - df["TightLepton_Eta[0]"])
    df[name+"_HadTopBTightLepton_dR"]   = np.sqrt(df[name+"_HadTopBTightLepton_dPhi"].values**2 + df[name+"_HadTopBTightLepton_dEta"].values**2)

    df[name+"_LepTopBTightLepton_dPhi"] = common.get_dPhi(df[name+"_LepTopB_Phi"].values, df["TightLepton_Phi[0]"])
    df[name+"_LepTopBTightLepton_dEta"] = abs(df[name+"_LepTopB_Eta"].values - df["TightLepton_Eta[0]"])
    df[name+"_LepTopBTightLepton_dR"]   = np.sqrt(df[name+"_LepTopBTightLepton_dPhi"].values**2 + df[name+"_LepTopBTightLepton_dEta"].values**2)

    df[name+"_LepTopBTightLepton_M"]   = vectors.get("LepTopBTightLepton", "M")
    df[name+"_HadTopBTightLepton_M"]   = vectors.get("HadTopBTightLepton", "M")
...
def def_signal_selection():
    sig_selection = [
    "bbfromttbar_HadTopB_CSV>0.277",
    "bbfromttbar_LepTopB_CSV>0.277",
    ]
