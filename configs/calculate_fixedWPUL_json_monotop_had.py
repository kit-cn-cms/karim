...
def calculate_variables(event, wrapper, sample, jec, dataEra = None, genWeights = None):
    ...
    for idx in range(getattr(event, "N_Jets_outside_lead_AK15Jet"+suffix)):
        ...
        if HadronFlav == 0:
            ...
            for wp in workingPoints:
                effs[wp] = data[year]["btagEff"].evaluate(wp, HadronFlav, eta, pt)
                jetsfs[wp]  = data[year]["mistagSF"].evaluate(wp, "central", HadronFlav, eta, pt)
                if jec == "nom":
                    for unc in jetuncs:
                        jetuncs[unc][wp] = data[year]["mistagSF"].evaluate(wp, unc, HadronFlav, eta, pt)
        else:
            if jec == "nom":
                for tagUnc in taggingUncertainties:
                    jetuncs[tagUnc] = {}
            ...
            for wp in workingPoints:
                effs[wp] = data[year]["btagEff"].evaluate(wp, HadronFlav, eta, pt)
                jetsfs[wp]  = data[year]["btagSF"].evaluate(wp, "central", HadronFlav, eta, pt)
                if jec == "nom":
                    for unc in jetuncs:
                        jetuncs[unc][wp] = data[year]["btagSF"].evaluate(wp, unc, HadronFlav, eta, pt)
