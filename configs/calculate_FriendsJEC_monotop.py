...
# vjets -> znunujets, wlnujets, dyjetstoll, gjets
# top -> ttbar, singletop
# signal_average -> combination of low mass, medium mass, and high mass monotop samples
samples = ["MC", "vjets", "top", "signal_average"]
...
for year in ["2016preVFP", "2016postVFP", "2017", "2018"]:
    ...
    for sample in samples:
        ...
        btagEffjson_had = correctionlib.CorrectionSet.from_file(os.path.join(sfDir, f"btagEff_monotop_had_deepJet_{sample}.json"))
        # btagEff[year]["lep"] = btagEffjson_lep["btagEff"]
        btagEff[year]["had"][sample] = btagEffjson_had["btagEff"]
...
def calculate_variables(events, wrapper, sample, jecs, dataEra = None, genWeights = None):
    ...
    # determine relevant sample efficiency
    if ("TTTo" in sample) or ("ST" in sample):
        relevant_sample = "top"
    elif ("ToNuNu" in sample) or ("WJetsToLNu" in sample) or ("DYJetsToLL" in sample) or ("G1Jet" in sample):
        relevant_sample = "vjets"
    elif ("TPhiTo2Chi" in sample) or ("Vector_MonoTop" in sample):
        relevant_sample = "signal_average"
    else:
        relevant_sample = "MC"
    print("relevant sample:", relevant_sample)
    ...
    eff_L_lfjets = btagEff[dataEra]["had"][relevant_sample].evaluate("L", flav_lfjets_flattened, eta_lfjets_flattened, pt_lfjets_flattened)
    eff_L_hfjets = btagEff[dataEra]["had"][relevant_sample].evaluate("L", flav_hfjets_flattened, eta_hfjets_flattened, pt_hfjets_flattened)

    # fail-safe
    eff_L_lfjets[eff_L_lfjets<=0.0]=0.001
    eff_L_lfjets[eff_L_lfjets>=1.0]=0.999
    eff_L_hfjets[eff_L_hfjets<=0.0]=0.001
    eff_L_hfjets[eff_L_hfjets>=1.0]=0.999
