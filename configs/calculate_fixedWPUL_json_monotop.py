...
def calculate_variables(event, wrapper, sample, jec, dataEra = None, genWeights = None):
    ...
    for idx in range(getattr(event, "nJets"+suffix)):
        ...
        if flav == 0:
            ...
            for wp in workingPoints:
                effs[wp] = data[year]["btagEff"].evaluate(wp, flav, eta, pt)
                jetsfs[wp]  = data[year]["mistagSF"].evaluate(wp, "central", flav, eta, pt)
                if jec == "nom":
                    for unc in jetuncs:
                        jetuncs[unc][wp] = data[year]["mistagSF"].evaluate(wp, unc, flav, eta, pt)
        else:
            if jec == "nom":
                for tagUnc in taggingUncertainties:
                    jetuncs[tagUnc] = {}
            ...
            for wp in workingPoints:
                effs[wp] = data[year]["btagEff"].evaluate(wp, flav, eta, pt)
                jetsfs[wp]  = data[year]["btagSF"].evaluate(wp, "central", flav, eta, pt)
                if jec == "nom":
                    for unc in jetuncs:
                        jetuncs[unc][wp] = data[year]["btagSF"].evaluate(wp, unc, flav, eta, pt)
