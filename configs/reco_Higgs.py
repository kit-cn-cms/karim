...
def calculate_variables(df):
    ...
    # angular differences
    df[name+"_Higgs_dPhi"]  = common.get_dPhi(df[name+"_B1_Phi"].values, df[name+"_B2_Phi"].values)
    df[name+"_Higgs_dEta"]  = abs(df[name+"_B1_Eta"].values - df[name+"_B2_Eta"].values)
    df[name+"_Higgs_dPt"]   = abs(df[name+"_B1_Pt"].values - df[name+"_B2_Pt"].values)
    df[name+"_Higgs_dR"]    = np.sqrt(df[name+"_Higgs_dEta"].values**2 + df[name+"_Higgs_dPhi"].values**2)
    df[name+"_Higgs_dKin"] = np.sqrt((df[name+"_Higgs_dEta"].values/5.)**2 + \
                                (df[name+"_Higgs_dPhi"].values/(2.*np.pi))**2 + \
                                (df[name+"_Higgs_dPt"].values/1000.))

    # reconstruct Higgs boson
    vectors = common.Vectors(df, name, ["B1", "B2"])
    vectors.add(["B1", "B2"], out = "Higgs")

    df[name+"_Higgs_Pt"]  = vectors.get("Higgs", "Pt")
    df[name+"_Higgs_Eta"] = vectors.get("Higgs", "Eta")
    df[name+"_Higgs_M"]   = vectors.get("Higgs", "M")
    df[name+"_Higgs_E"]   = vectors.get("Higgs", "E")
    df[name+"_Higgs_Phi"]   = vectors.get("Higgs", "Phi")

    # log values
    for obj in ["B1", "B2", "Higgs"]:
    ...
    # 3D opening angle
    df[name+"_Higgs_openingAngle"] = vectors.getOpeningAngle("B1", "B2")

    # boost
    vectors.boost(["B1", "B2", "Higgs"], frame = "Higgs")

    # add boosted variables
    for obj in ["B1", "B2", "Higgs"]:
        df[name+"_"+obj+"_Pt_boosted"] = vectors.get(obj, "Pt", boostFrame = "Higgs")
        df[name+"_"+obj+"_M_boosted"] = vectors.get(obj, "M", boostFrame = "Higgs")
        df[name+"_"+obj+"_E_boosted"] = vectors.get(obj, "E", boostFrame = "Higgs")
        df[name+"_"+obj+"_Eta_boosted"] = vectors.get(obj, "Eta", boostFrame = "Higgs")
        df[name+"_"+obj+"_Phi_boosted"] = vectors.get(obj, "Phi", boostFrame = "Higgs")
    ...
    #get delta variables of lepton and Higgs-boson
    df[name+"_Higgs_dEta_lept"] = common.get_dEta(df["TightLepton_Eta[0]"].values,df[name+"_Higgs_Eta"].values)
    df[name+"_Higgs_dPhi_lept"] = common.get_dPhi(df["TightLepton_Phi[0]"].values,df[name+"_Higgs_Phi"].values)

    df[name+"_Higgs_dR_lept"] = common.get_dR(
         df["TightLepton_Eta[0]"].values,
         df["TightLepton_Phi[0]"].values,
         df[name+"_Higgs_Eta"].values,
         df[name+"_Higgs_Phi"].values)



    #get average of btag-values of the two b-jets
    df[name+"_Higgs_btagAverage"] = (df[name+"_B1_btagValue"].values + df[name+"_B2_btagValue"].values) / 2.
